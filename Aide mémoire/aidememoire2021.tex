...
\subsection{fonction $ln(x)$ et $e$}
\subsubsection{$e$, le nombre d'Euler}
On pose par définition :
\begin{center}
    \[ e^x = \sum_{n=0}^{+\infty} \frac{x^n}{n!} \]
\end{center}
On nomme \textbf{fonction exponentielle} la fonction exp :${R}\rightarrow \mathbb{R}^{*}_{+}$ définie par $ exp(x)=e^x$.
Elle est continue, strictement croissante et bijective.\\
...
\begin{itemize}
    ...
    \item $e^{x+y}=e^xe^y$
    \item $e^{-x}= \frac{1}{e^x}$
    \item$ \lim_{x\to-\infty} e^x = 0$ et $\lim_{x\to+\infty} e^x= +\infty $
\end{itemize}
\subsubsection{$ln$ : logarithme naturel, aussi appelé logarithme népérien}
le logarithme népérien est un logarithme dit "de base $e$", car $ln(e)=1$. Cette fonction est la fonction réciproque de la fonction exponentielle.
Elle est donc une fonction continue strictement croissante ln : ${R}\rightarrow \mathbb{R}^{*}_{+}$. Ainsi, par défininition
\[y = ln(x) \Longleftrightarrow x = e^y\]
\subsubsection{Propriétés}
$\forall x\in \mathbb{R} , y\in \mathbb{R}^{*}_{+}  : $
\begin{itemize}
    \item $ln(e^x) = x$
    \item $ln(1)=0$ et $ln(e) =1$
    \item $ln(xy) = ln(x) + ln(y)$ et $ln(\frac{x}{y})= ln(x) -ln(y)$
    \item $\lim_{x\to0^+} ln(x) = -\infty$ et $\lim_{x\to+\infty} ln(x)= +\infty $
    \item $a^x = e^{xln(a)}$
\end{itemize}
...
Soit $(a_n)$ une suite d'éléments de $\mathbb{R}$. Toute expression de la forme :
\begin{center}
    $$\sum_{n=0}^{+\infty}a_n$$
\end{center}
est appellée une \textbf{série numérique}.
\begin{definition}
    On dit de la série $\sum_{n=0}^{+\infty}a_n$ qu'elle est \textbf{convergente} si la suite des \textbf{sommes partielles :}
        $$(S_p=\sum_{n=0}^{+\infty}a_n)$$
    converge. La limite de cette suite, lorsqu'elle existe, est appelée la \textbf{somme} de la série, et on pose, par définition :
        $$\sum_{n=0}^{+\infty}a_n = \lim_{p \to \infty} S_p$$
    Lorsque la suite $S_p$ diverge, on dit que la série $\sum_{n=0}^{+\infty}a_n$ \textbf{diverge}. Dans le cas particulier ou $\lim_{p \to \infty} S_p = +\infty$ (resp. $-\infty$), on écrit: $\sum_{n=0}^{+\infty}a_n=+\infty$  (resp. $-\infty$).
\end{definition}
\begin{definition}
    On dit de la série $\sum_{n=0}^{+\infty}a_n$ qu'elle est \textbf{absolument convergente} si la série $\sum_{n=0}^{+\infty} \lvert a_n \rvert$ est convergente.
\end{definition}

\subsection{Critères de convergence}
\subsubsection{Critère du quotient}
Soient $(a_n)$ et $(b_n)$ deux suites de nombres réels positifs telles que 
$$\lim_{n \to \infty} \frac{a_n}{b_n} = l \in \mathbb{R}^{*}_{+}$$
alors deux cas se présentent :\\
Soit $\sum_{n=0}^{+\infty}a_n$ et $\sum_{n=0}^{+\infty}b_n$ sont convergentes, soit les deux divergent.
\subsubsection{Critère de d'Alembert}
Soit $(a_n)$ une suite d'éléments de $\mathbb{R}$ pour laquelle
$$\lim_{n \to \infty} \lvert \frac{a_{n+1}}{a_n} \rvert = \rho$$
Alors, si $\rho < 1$, la série $\sum_{n=0}^{+\infty}a_n$ est absolument convergente, et diverge si $\rho >1$. 
\subsubsection{Critère de Cauchy}
Soit $(a_n)$ une suite d'éléments de $\mathbb{R}$ et 
$$\lim_{n \to \infty} \sqrt[n]{\lvert a_n \rvert} = l = \rho$$
Alors, si $\rho <1$, la série $\sum_{n=0}^{+\infty}a_n$ est absolument convergente, et diverge si $\rho > 1$.
\subsubsection{Critère des séries alternées}
Soit $(a_n)$ une suite d'éléments de $\mathbb{R}$ vérigiant les trois propriétés suivantes :\\
\begin{itemize}
    \item $\forall n \in \mathbb{N} : a_n * a_{n+1} \leq 0$
    \item $\forall n \in \mathbb{N} : \lvert a_{n+1} \rvert \leq \lvert a_n \rvert$
    \item $\lim_{n \to \infty} a_n = 0$
\end{itemize}
Alors, la série $\sum_{n=0}^{+\infty}a_n$ converge. De plus, pour tout entier $p \geq 0$ :
$$\lvert \ \sum_{n=0}^{+\infty}a_n \rvert \leq \lvert a_p \rvert$$
...
Soient $I$ un intervalle ouvert, (TODO)
\newpage
...
\end{center}
est appelé un \textbf{changement de variable.}\footnote{Il est nécéssaire de regarder des exemples afin de mieux comprendre le fonctionnement de cette méthode.}
...
\end{itemize}
\begin{remarque}
    Il est important de se rappeler de changer $dx$ par $dt$, que l'on determine avec :
    \begin{center}
        $\frac{dt}{dx} = \varphi(t)'$
    \end{center}
 \end{remarque}
...
TODO (? -> pas forcément utile pour le BA1)
\newpage
\section{Equations Différentielles}
TODO
\newpage
\chapter{Algèbre Linéaire}
\section{Equations linéaires en algèbre linéaire}
\subsection{Systèmes d'équations linéaires}
Une équation est dite \textbf{linéaire} d'inconnues $x_1, x_2, ... , x_n$ si on peut l'arranger de la forme :
\begin{center}
    $a_1x_1 + a_2x_2 + ... + a_nx_n = b$
\end{center}
avec $b$ et les \textbf{coefficients} $a_1, ... ,a_n$ des nombres réels ou complexe dont on connaît (en général) la valeur.
$n$ est strictement positif.\\
On appelle \textbf{solution} du système toute liste $(s_1, s_2, ... ,s_n)$ de nombres qui transforme chaque équation en une égalité
vraie quand on substitue $s_1, ... ,s_n$ respectivement à $x_1, ... , x_n$.\\
On appelle \textbf{ensemble des solutions} l'ensemble de toutes les solutions posssibles du système linéaire.\\
On dit de deux systèmes qui partagent le même ensemble de solutions qu'ils sont \textbf{équivalents} (toute solution d'un système est solution du second et vice-versa).\\
Un système est dit \textbf{compatible (ou consistant)} si il adment au minimum une solution.\\
Dans le cas inverse, on le dit \textbf{incompatible (ou incosistant)} si il ne possède pas de solution.
\newpage
\subsection{Notation matricielle}
On peut présenter les informations importantes dans un tableau appelé \textbf{matrice}. Par exemple, étant donné le système :
\begin{center}
    $ax_1+bx_2+cx_3 = u$\\
    $dx_1+ex_2+fx_3 = v$\\
    $gx_1+hx_2+ix_3 = w$
\end{center}
on peut représenter la \textbf{matrice des coefficients du système}
$$ M = \begin{pmatrix}
  a & b & c \\
  d & e & f \\
  g & h & i
\end{pmatrix}
$$
ainsi que la \textbf{matrice complète (ou matrice augmentée)} du système :
$$ M = \begin{pmatrix}
    a & b & c & u\\
    d & e & f & v\\
    g & h & i & w
